...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (cpForm) {
    cpForm.onsubmit = async (e) => {
      e.preventDefault();
      const cpOld = document.getElementById('cpOld').value;
      ...
      cpBtnText.textContent = 'UPDATING...';
      cpSpinner.style.display = 'inline-block';
      ...
      try {
        ...
        cpBtnText.textContent = 'UPDATE PASSWORD';
        cpSpinner.style.display = 'none';
      }
    };
  }
  ...
  document.querySelectorAll('.nav-item').forEach(item => {
    ...
    item.onclick = () => {
      ...
      if(dishesView) dishesView.style.display = view === 'dishes' ? 'block' : 'none';
      if(ordersView) ordersView.style.display = view === 'orders' ? 'block' : 'none';
      if(categoriesView) categoriesView.style.display = view === 'categories' ? 'block' : 'none';
      if(chefView) chefView.style.display = view === 'chef' ? 'block' : 'none';
      if (view === 'analytics') fetchAnalytics();
      else if (view === 'dishes') fetchDishes();
  ...
  function initCharts() {
    Chart.defaults.color = 'rgba(240, 236, 228, 0.6)';
    ...
    if (revCtx) {
      charts.revenue = new Chart(revCtx, {
        type: 'line',
        data: { labels: [], datasets: [{ label: 'Revenue ($)', data: [], borderColor: '#d4af37', backgroundColor: 'rgba(212, 175, 55, 0.1)', fill: true, tension: 0.4 }] },
    ...
    if (statCtx) {
      charts.status = new Chart(statCtx, {
        type: 'doughnut',
        data: { labels: ['Delivered', 'Cancelled', 'Pending'], datasets: [{ data: [0,0,0], backgroundColor: ['#22c55e', '#ef4444', '#d4af37'], borderWidth: 0 }] },
    ...
    if (prodCtx) {
      charts.products = new Chart(prodCtx, {
        type: 'bar',
        data: { labels: [], datasets: [{ label: 'Qty Sold', data: [], backgroundColor: '#d4af37', borderRadius: 6 }] },
  ...
  async function fetchAnalytics() {
    ...
    // Auto fetch when picker changes
    picker.onchange = () => {
      fetchAnalytics();
    };
    ...
    // Dynamic Clear Button UI
    const clearBtnText = document.getElementById('clearAnalyticsMonthText');
    const clearBtn = document.getElementById('clearAnalyticsMonthBtn');
    if (clearBtnText) {
      const d = new Date(year, parseInt(month) - 1);
      const monthName = d.toLocaleString('en-US', { month: 'long' });
      clearBtnText.textContent = `Clear ${monthName} ${year}`;
    }
    ...
    if (clearBtn) {
      clearBtn.onclick = async () => {
        ...
        const confirmMsg = `Are you sure you want to delete all records for ${monthName} ${year}?\nThis will hide the data from Analytics indefinitely.`;
        if(!confirm(confirmMsg)) return;

        try {
          const res = await authFetch(`${API_URL}/analytics?month=${month}&year=${year}`, {
            method: 'DELETE'
          });
          if (res.ok) {
            const data = await res.json();
            alert(`Success! Soft-cleared ${data.count} records for ${monthName} ${year}.`);
            fetchAnalytics(); // Immediately update dashboard state to 0
          } else {
            alert('Failed to clear month records.');
          }
        } catch(err) {
          console.error('Error clearing data', err);
          alert('Server error while clearing data.');
        }
      };
    }
    ...
    try {
      ...
      const setGrowth = (elId, value) => {
        const el = document.getElementById(elId);
        if(!el) return;
        const color = value >= 0 ? '#22c55e' : '#ef4444';
        const sign = value > 0 ? '+' : '';
        el.innerHTML = `<span style="color:${color}">${sign}${value.toFixed(1)}%</span> from last month`;
      };

      if(document.getElementById('statRevenue')) {
        document.getElementById('statRevenue').textContent = `Rs. ${(data.stats.totalRevenue * RATE).toLocaleString()}`;
        ...
        document.getElementById('statAOV').textContent = `Rs. ${(data.stats.aov * RATE).toLocaleString()}`;

        setGrowth('growthRevenue', data.growth.revenue);
        setGrowth('growthOrders', data.growth.orders);
        setGrowth('growthCustomers', data.growth.customers);
        setGrowth('growthAOV', data.growth.aov);
      }

      // Update Charts
      if (!charts.revenue) initCharts();
      if (charts.revenue && data.monthlySales) {
        charts.revenue.data.labels = data.monthlySales.map(m => m.month);
        charts.revenue.data.datasets[0].data = data.monthlySales.map(m => m.revenue * RATE);
        charts.revenue.update();
      }
      if (charts.status && data.statusCounts) {
        charts.status.data.datasets[0].data = [data.statusCounts.Delivered, data.statusCounts.Cancelled, data.statusCounts.Pending];
        charts.status.update();
      }
      if (charts.products && data.topProducts) {
        charts.products.data.labels = data.topProducts.map(p => p.name);
        charts.products.data.datasets[0].data = data.topProducts.map(p => p.count);
  ...
  window.initDashboard = () => {
    ...
    if (analyticsView && analyticsView.style.display !== 'none') {
      ...
      fetchCategories().then(() => { if (dishesView && dishesView.style.display !== 'none') fetchDishes(); });
    }
    setupSSE();
  };
  ...
  function createOrderRow(order) {
    ...
    return `<tr id="order-${order.id}">
      ...
      <td>${formatDateStandard(order.date)}</td>
      <td>${order.customerName}</td>
      <td>${order.customerPhone || 'N/A'}</td>
      <td>${order.tableNumber || 'N/A'}</td>
      <td>${order.items.map(i => `<span style="font-weight:600;">${i.quantity}x</span> ${i.name}`).join('<br>')}</td>
      ...
      <td>
        <div style="display:flex;gap:8px;">
          ...
          <button class="btn-danger" style="padding:6px;border-radius:4px;font-size:0.7rem;" onclick="deleteRemoteOrder('${order.id}')" title="Soft Delete">DEL</button>
        </div>
      </td>
    </tr>`;
  }
  ...
  function openPrintPreview(order) {
    ...
    const orderIdStr = '#' + String(order.id).slice(-4);
    
    // --- Invoice Population ---
    document.getElementById('p-order-id').textContent = orderIdStr;
    document.getElementById('p-date').textContent = now.toLocaleDateString('en-GB');
    ...
    `).join('');
    document.getElementById('p-items-container').innerHTML = itemsHTML;
    ...
    // --- KOT Population ---
    document.getElementById('kot-order-id').textContent = 'ORDER ' + orderIdStr;
    document.getElementById('kot-pickup-type').textContent = order.tableNumber ? 'DINE-IN' : 'DELIVERY';
    
    document.getElementById('kot-time').textContent = now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
    
    const tableContainer = document.getElementById('kot-table-container');
    if (order.tableNumber) {
      document.getElementById('kot-table').textContent = order.tableNumber;
      tableContainer.style.display = 'block';
    } else {
      tableContainer.style.display = 'none';
    }
    
    const notesStr = order.specialInstructions ? `<div style="font-size:14pt; margin-top:6px; font-style: italic; border: 2px dashed #000; padding: 6px; display: inline-block; font-weight: bold;">(Note: ${order.specialInstructions}...)</div>` : '';
    
    const kotItemsHTML = order.items.map(it => `
      <div style="margin-bottom: 15px; display: flex; flex-direction: column; align-items: flex-start;">
        <div style="font-size: 20pt; line-height: 1.2;">
          <span style="font-weight: 900; font-size: 24pt;">${it.quantity} x</span> <span style="font-weight: bold;">${it.name}</span>
        </div>
        ${notesStr}
      </div>
    `).join('');
    
    document.getElementById('kot-items-container').innerHTML = kotItemsHTML;

    // Show Preview Modal
    console.log("[Admin] Opening print preview for order:", order.id);
    const modal = document.getElementById('receipt-preview-modal');
    modal.className = 'show'; // Clear any print CSS classes
  ...
  if (confirmPrintBtn) {
    confirmPrintBtn.onclick = () => {
      const modal = document.getElementById('receipt-preview-modal');
      modal.className = 'show show-invoice';
      window.print();
    };
  }
