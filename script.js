...
document.addEventListener('DOMContentLoaded', () => {
    ...
    container.innerHTML = filtered.map(dish => `
      <div class="dish-card" onclick="openProductView('${dish.id}')">
        ...
        <div class="dish-info">
          ...
          <p class="dish-calories">${dish.calories ? dish.calories + ' cal' : ''}</p>
  ...
  window.openProductView = (dishId) => {
    ...
    if (!dish) return;
    getById('prodImage').src = dish.image; getById('prodTitle').textContent = dish.name; getById('prodTypeIcon').textContent = dish.type === 'veg' ? '🟢' : '🔴'; getById('prodPrice').textContent = formatPrice(dish.price).replace(/^\D+\s?/, ''); getById('prodCalories').textContent = dish.calories ? dish.calories + ' cal' : ''; getById('prodIngredients').innerHTML = dish.ingredients.map(ing => `<div class="ing-pill">${ing}</div>`).join('');
